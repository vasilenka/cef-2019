  ...
  return to ? (
    <div className={className}>
      <a href={to} target="_blank" rel="noopener noreferrer">
        <div className={cx(styles.partner, chula ? styles.chula : null)}>{children}</div>
        {name && (
          <Text heading6 className={styles.partnerName}>
            {name}
          </Text>
        )}
      </a>
...
const FundingPartners = ({ className, ...restProps }) => {
  return (
    <>
        ...
        <Container narrow>
          <Box column className={styles.mainPartner}>
            ...
            <Box justifyCenter alignCenter className={styles.partnerRow}>
              <PartnerContainer
                className={cx(styles.partnerFunding)}
                chula
                children={<Chulalongkorn />}
                to="https://www.chula.ac.th/en/"
              />
              <PartnerContainer
                className={styles.partnerFunding}
                children={<Ford />}
                to="https://www.fordfoundation.org/"
              />
              <PartnerContainer
                className={styles.partnerFunding}
                children={<AsiaCenter />}
                to="https://www.jpf.go.jp/e/index.html"
              />
              <PartnerContainer
                className={styles.partnerFunding}
                children={<Solo />}
